<?php
$GLOBALS['primaryCommandforca']->registerSubCommand('edit', function($message, $params){
	global $discord, $bot_cfg;
	$msg = "**Editar palavra no Jogo da Forca!**";
	...
	if($message->channel_id !== $bot_cfg['admin_channel_id']){
		...
		$embed = [
			...
			'title' => 'Ops! Erro ao editar a palavra!',
			'description' => "Digite o comando corretamente: ``\$forca edit \"<palavra>\" \"<chances>\" \"<dica_1>\"(N)``",
			'thumbnail' => [
		...
		if(
			...
			if(!$exist || is_null($key)){
				$embed['description'] = "Não existe a palavra '{$params[0]}' cadastrado!\nDigite ``\$forca edit \"<palavra>\" \"<chances>\" \"<dica_1>\"(N)`` para criar um novo";
			}else{
				...
				$arr[$key] = $palavras_to_save;
				
				$encoded = json_encode($arr, JSON_PRETTY_PRINT);
				
				if(file_put_contents('uploads/forca.json', $encoded)){
					$embed['title'] = 'Palavra editado com sucesso!';
	...
	'usage' => "\$forca edit \"<palavra>\" \"<chances>\" \"<dica_1>\"(N)",
